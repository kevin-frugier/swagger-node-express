  ...
  _.forOwn(properties, function (property) {
    ...
    if(type) {
      ...
      if (property.$ref){
        ...
        if (!self.allModels[property.$ref]) {
...
Swagger.prototype.addMethod = function(app, callback, spec) {
  ...
  var apiRootPath;
  var apiVersion = self.apiVersion;
  console.log(spec.path);
  var match = spec.path.match(apiRe); 
  if (match) {
    apiVersion = match[1];
    apiRootPath = 'api/v' + apiVersion;
  } else {
    apiRootPath = spec.path.split(/[\/\(]/)[1];
  }
